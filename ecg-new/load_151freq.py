...
from models_test.endecoder import *
...
def validation(model,validation_loader,device,optimizer,epoch,freq):
    model.eval()
    losses = []
    scores = []
    N_count = 0
    ...
    with torch.no_grad():
        ...
        print('freq = {}, validation epoch:{},loss:{:.5f},accuracu:{}'.format(freq,epoch, sum(losses) / N_count,
                                                                         sum(scores) / N_count))
        with open ('result_split_64_2.txt','a+') as f:
            f.write('freq = {}, validation epoch:{},loss:{:.5f},accuracu:{}'.format(freq,epoch, sum(losses) / N_count,
                                                                         sum(scores) / N_count))
            f.write('\n')
...
if __name__ == '__main__':
    ...
    ckpt_path='./ckpt/select'
    if not os.path.exists(ckpt_path):
        os.mkdir(ckpt_path)
    epochs=50
    learning_rate=0.001

    frequency=[i for i in range(64)]
    for freq in frequency:

        model = ResNet(BasicBlock, [2, 2, 2, 2],num_classes=3,freq=[freq]).to(device) #resnet152() 1-D卷积分类网络

        #model.load_state_dict(torch.load('./ckpt/resnet/epoch_3.pth'))
        ...
        for epoch in range(epochs):
            ...
            validation(model,test_loader,device,optimiezer,epoch,freq)
