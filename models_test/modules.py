...
def get_freq_indices(method):
    ...
    if 'top' in method:
        all_top_indices_x = [i for i in range(157)]
...
class MultiSpectralDCTLayer(nn.Module):
    def __init__(self,channel,length_feature,reduction=16,freq_sel_method = 'top1'):
        ...
        mapper_x = [temp_x * (length_feature // 157) for temp_x in mapper_x]
    ...
    def get_dct_filter(self,length_feature,mapper_x,channel):
        ...
        for i,u_x in enumerate(mapper_x):
            for t_x in range(length_feature):
                dct_filter[i*c_part:(i+1)*c_part]=self.build_filter(t_x,u_x,length_feature)
        return dct_filter
